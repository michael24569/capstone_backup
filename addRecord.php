...
// Add URL parameter handling after session_start()
if (isset($_GET['lotno']) && isset($_GET['memsts']) && isset($_GET['memlot'])) {
    $lot = $_GET['lotno'];
    $mem_sts = $_GET['memsts'];
    $mem_lots = $_GET['memlot'];
}
...
if (isset($_SERVER['HTTP_REFERER'])) {
    ...
    if (strpos($referer, 'home.php') !== false) {
        $_SESSION['return_to'] = 'home.php';
    } else if (strpos($referer, 'records.php') !== false) {
        $_SESSION['return_to'] = 'records.php';
    } else {
        $_SESSION['return_to'] = 'records.php'; // default fallback
    }
}
...
<body>
    ...
    <script>
        ...
        // Update the goBack function to handle URL parameters
        function goBack() {
            // Get URL parameters
            const urlParams = new URLSearchParams(window.location.search);
            const fromMap = urlParams.has('lotno'); // Check if we came from the map

            if (fromMap) {
                window.location.href = "home.php";
            } else {
                window.location.href = "<?php echo $_SESSION['return_to'] ?? 'records.php'; ?>";
            }
        }
    </script>
</body>
